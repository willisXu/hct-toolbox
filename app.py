...
with tab_compare:
    ...
    st.markdown(
        ...
        "產出數量核對與訂單編號核對結果。\n\n"
        "數量核對表除了「效期」（yyyy-mm-dd）另留一欄「批號」，"
        "用批號原始寫法（yyyymmdd）以**文字格式**輸出，可直接複製回系統查批。"
    )
...
with tab_return:
    ...
    st.markdown(
        ...
        "明細表除了「效期」（yyyy-mm-dd）另留一欄「批號」，"
        "用批號原始寫法（yyyymmdd）以**文字格式**輸出。"
    )
    col1, col2 = st.columns(2)
    with col1:
...
with tab_inventory:
    ...
    st.markdown(
        ...
        "庫存量＝數量＋組合保留）。\n\n"
        "HCT 與代工廠對帳**不限定倉別**：報表有什麼倉別就核對什麼，新倉別不必改程式"
        "（代工廠的合計／總計列仍自動排除，「庫存數量」同時當作可出與庫存數量）。"
        "M00 對帳例外，只核對 **M00 開頭倉別**——M00 報表沒有倉別欄、整份視為 M00 倉，"
        "NetSuite 側若不限定，每筆非 M00 倉都會變成假的「僅 NetSuite 存在」。\n\n"
        "不限定倉別的代價：NetSuite 報表匯出範圍要自己抓好，匯出到不相干的倉別時，"
        "那些列會列成「僅 NetSuite 存在」。差異基準為 **HCT／代工廠／M00－NetSuite**。\n\n"
        "「日期明細」分頁除了「到期日」另留一欄「批號」，"
        "用批號原始寫法（yyyymmdd）以**文字格式**輸出；"
        "「料號彙總」跨批號加總，因此不含批號欄。"
    )
    col1, col2 = st.columns(2)
